...
"""
        ...
        self.commands = {
            ...
            "ver": lambda command: self.text_area.insert(END, f"\nShellyOS Terminal v.{ver} 未經授權可以轉載"),
...
class UpdateLog():
    def __init__(self, master):
        ...
        self.combo['values'] = ("ver 3.1.415.926","ver 3.0.2333.38","ver 3.0.2333.38β","ver 2.7.1919.810","ver 2.5.1616.69", "ver 2.0.114.514", "ver 1.0.114.514")
        self.combo.current(0)  # 預設選第一個
        self.combo.pack(pady=5)
...
class Settings():
    def __init__(self, master):
        ...
        self.combo['values'] = ("預設", "城市夜晚", "海面", "經典", "塗鴉", "自然")
        self.combo.current(["default","night","sea","basic","doodle","nature"].index(theme))
...
class Taskman():
    def __init__(self, master):
        ...
        self.icon = PhotoImage(file=r"X:\bin\icons\defwin.png")
        self.btn = Button(root, anchor='w',bg=bgc,fg=fgc,image=self.icon, command=self.focus_window)
        self.btn.image = self.icon #以防回收
        self.btn.pack(side='left')
        appbar_apps.append(self.btn)
        
        self.got = [type(app).__name__ for app in apps_running]
        self.window = Toplevel(master)
        ...
        self.title_label = Label(self.title_bar, text="工作終止工具", bg="#eeffff", fg="black", font=("微軟正黑體", 12))
        self.title_label.pack(side="left", padx=5)
        self.exit_btn = Button(self.title_bar, text="✕", command=self.exiting)
        self.exit_btn.pack(side="right")
        ...
        self.listbox = Listbox(self.window, selectmode="browse", height=20, width=80, font=("微軟正黑體", 12))
        self.listbox.pack(pady=5)
        self.listbox.bind("<Double-Button-1>", self.confirm_exit)

        self.change_color()
        self.refresh()
        self.window.after(10,self.loop_get)
    
    def confirm_exit(self,event):
        selection = self.listbox.curselection()
        if MsgBox.ask(self.window, "確定", "您真的要關閉此程式嗎？"):
            if self.listbox.get(selection[0]) == "Kernel":
                crash("CORE_PROCESS_LEFT")
                return
            else:
                apps_running[selection[0]].exiting()
        
    def loop_get(self):
        after_get = [type(app).__name__ for app in apps_running]
        if self.got != after_get:
            self.got = after_get
            self.refresh()
        self.window.after(500,self.loop_get)
            
    def refresh(self):
        self.listbox.delete(0, "end")
        for i in self.got:
            self.listbox.insert("end", i)
        self.listbox.insert("end", "Kernel")
    def start_move(self, event):
        self.x_offset = event.x
        self.y_offset = event.y
...
def change_theme(t):
    ...
    walls = {
        ...
        "night": ["night1.png", "night2.png", "#C9B0A9","#BDE8CD","#333","#fff"],
        ...
        "nature": ["nature1.png", "nature2.png", "#CFEBD4","#BDE1D5","#BDE8CD","#6D6771"]
    }
...
def runapp(e):
    ...
    if e == "note":
        ...
        apps_running.append(Taskman(root))
    elif e == "settings":
...
def crash(code):
    time.sleep(1) #這人可真會戲劇效果啊
    root.destroy()
    popen(popen(f'X:\\bin\\died.exe {code}'))
    exit()

def initialization():
    global log_file, timer, root, appbar_apps, apps_running, shutdown_bg, shutdown_window, start, start_open, config_file, ver
